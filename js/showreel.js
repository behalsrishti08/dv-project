var m = [20, 20, 30, 20],
    w = $("#showreel").width() - m[1] - m[3],
    h = $("#showreel").width() - m[0] - m[2];
...
var svg_showreel = d3.select("#showreel").append("svg")
...
d3.csv("D3Showreel.csv", function(data) {
  ...
  var g = svg_showreel.selectAll("g")
      .data(symbols)
    .enter().append("g")
      .attr("class", "symbol");
...
function lines() {
  ...
  var k = 0, n = symbols[0].values.length - 1;
  d3.timer(function() {
    draw(k);
    if ((k += 1) >= n - 1) {
      draw(n - 1);
...
function horizons() {
  svg_showreel.insert("defs", ".symbol")
    .append("clipPath")
      .attr("id", "clip")
    .append("rect")
      .attr("width", w)
...
function areas() {
  ...
  svg_showreel.select("defs").transition()
      .duration(duration)
      .remove();
...
function stackedArea() {
  ...
  var t = svg_showreel.selectAll(".symbol").transition()
      .duration(duration)
...
function transposeBar() {
  ...
  svg_showreel.select("line").transition()
      .duration(duration)
      .attr("x2", w);
...
function donutExplode() {
  ...
  svg_showreel.selectAll(".symbol path")
  ...
  setTimeout(function() {
    svg_showreel.selectAll("*").remove();
    svg_showreel.selectAll("g").data(symbols).enter().append("g").attr("class", "symbol");
    lines();
  }, duration);
}
